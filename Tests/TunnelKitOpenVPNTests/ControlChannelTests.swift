...
//    00000000 // message packet_id
    func testAuth() {
        let client = try! OpenVPN.VpnDirection.parseValidation(withKey: OpenVPN.FixedCreds(data: Data(hex: hex), direction: .client), digest: .sha512)
        let server = try! OpenVPN.VpnDirection.parseValidation(withKey: OpenVPN.FixedCreds(data: Data(hex: hex), direction: .server), digest: .sha512)
...
//        let original = Data(hex: "38bccfd1")
    ...
    func testCrypt() {
        let client = try! OpenVPN.VpnDirection.EncryptionParsing(withKey: OpenVPN.FixedCreds(data: Data(hex: hex), direction: .client))
        let server = try! OpenVPN.VpnDirection.EncryptionParsing(withKey: OpenVPN.FixedCreds(data: Data(hex: hex), direction: .server))
